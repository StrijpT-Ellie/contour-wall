    ...
    def show(self, sleep_ms:int=0, optimize:bool=True):
        for row in range(self.rows):
            for col in range(self.cols):
                ...
                cv2.rectangle(self.matrix, top_left, bottom_right, color,-1)

                border_color = (0, 0, 0) 
                cv2.rectangle(self.matrix, top_left, bottom_right, border_color, 1)
                self.pushed_frames += 1

        cv2.imshow('Contour Wall Emulator', self.matrix)
        cv2.waitKey(1)
