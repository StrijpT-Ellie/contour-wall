...
def draw_palm_box(frame, palm_landmarks, scale_factor=1, output_size=(20, 20), upscale_factor=5):
    ...
    if not palm_landmarks:
        ...
        # Upscale the image
        upscaled_frame = cv2.resize(black_frame_resized,
    ...
    while True:
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                upscaled_frame = draw_palm_box(frame, hand_landmarks.landmark, output_size=(20, 20), upscale_factor=50)
                cv2.imshow("Hand Tracking", upscaled_frame)
        else:
            # Show a black screen when no landmarks are found
            black_frame_same_size = draw_palm_box(frame, [], output_size=(20, 20), upscale_factor=50)
